...
Edit `config.js` and refresh.

#### Google Places API (optional)

Set the **`GOOGLE_PLACES_API_KEY`** environment variable on your server (e.g. Vercel → Project → Settings → Environment Variables). The key is used exclusively by server-side proxy functions (`/api/places/*`) and **never** shipped to the browser. When configured, the "Your Favorite Spots" section shows place names, addresses, and photos. Without the key, only the "Open in Google Maps" link is shown.
...
├── api/
│   └── places/
│       ├── search.js    # Serverless proxy: Google Places searchNearby (key stays server-side)
│       └── photo.js     # Serverless proxy: Google Places photo media
├── data/
│   └── countries.geojson
├── components/
├── tests/
...
- **Build**: `npm run build` (builds Tailwind → `tailwind.css`).
- **Headers**: Custom `Permissions-Policy` in `vercel.json` (no `browsing-topics`).
- **Output**: Static files from repo root + serverless functions in `api/`.
- **Google Places API (production)**: In Vercel → Project → **Settings** → **Environment Variables**, add `GOOGLE_PLACES_API_KEY` with your key. It is used server-side only by the proxy functions in `api/places/` and never reaches the browser.
